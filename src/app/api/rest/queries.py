...
def query(request: QueryRequest, http_request: Request):
    ...
    try:
        return http_request.app.state.service.execute(request.sql, source="rest", request_id=request_id)
...
def validate(request: QueryRequest, http_request: Request):
    request_id = http_request.headers.get("x-request-id", str(uuid4()))
    try:
        plan = http_request.app.state.service.validate(request.sql, source="rest", request_id=request_id)
    except (ValueError, TimeoutError) as exc:
